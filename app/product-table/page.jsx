...
import ProductTable from "/components/shop-elements/ProductTable";
...
export default async function page({ params, searchParams }) {
  ...
  var per_page = searchParams?.per_page; // 👈 gets ?category=baby-care




  var queryPrams = { category: searchParams?.category, brand: searchParams?.brand, tag: searchParams?.tag, keyword: "", paged: 1, order: "DESC", per_page: per_page, price: "" };



  var postData = JSON.stringify(queryPrams);

  const res = await fetch(`${serverUrl}wp-json/promptshub/v2/get_product_table`, {
    method: "POST",
  ...
  var productsData = { posts: data?.posts, total: data?.total, maxPages: data?.max_pages };
  // var [productsData, setpromptsData] = useState({ posts: data?.posts, total: data?.total, maxPages: data?.maxPages });
  ...
  return (
    ...
    <div>
      ...
      <ProductTable queryPrams={queryPrams} entries={productsData}
